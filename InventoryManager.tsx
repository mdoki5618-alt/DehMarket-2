...
export default function InventoryManager({
  ...
  const openAddForm = () => {
    ...
    setId(''); // leave empty: force scanning/typing the real barcode to avoid mismatches at checkout
    setName('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!finalId) {
      const confirmRandom = window.confirm(
        '⚠️ شما بارکدی وارد نکردید. اگر ادامه بدید، یک کد تصادفی برای این کالا ساخته میشه که با بارکد چاپ‌شده روی بسته یکی نیست و موقع اسکن واقعی شناسایی نمیشه.\n\nآیا مطمئنید می‌خواید بدون بارکد واقعی ادامه بدید؟'
      );
    ...
    if (editingProduct) {
      ...
      // Check duplicate barcode (digit-normalized so Persian/English numerals of the same code match)
      const cleanPayloadId = normalizeBarcode(payload.id).toLowerCase();
      if (products.some((p) => normalizeBarcode(p.id).toLowerCase() === cleanPayloadId)) {
            ...
            <div className="my-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  />
                  {!editingProduct && (
                    <span className="text-[10px] text-gray-400">بارکد روی بسته کالا رو با دستگاه اسکنر بزنید یا دستی تایپ کنید. اگر خالی بذارید، هنگام ذخیره هشدار داده میشه.</span>
                  )}
                </div>
