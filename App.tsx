...
import { getProductImageUrl } from './components/imageHelper';
import { normalizeBarcode } from './components/barcodeHelper';
...
export default function App() {
  ...
  // NOTE: Automatic background online-image resolution was removed.
  // It ran a fetch-per-product loop (with a delay) every time the product
  // count changed, which froze the UI for a long time after bulk-adding
  // inventory, and it depends on internet access which this app should
  // not rely on. Product images now only use the local/offline lookup
  // (getProductImageUrl) and any manually-entered image URL.
  ...
  const handleQuickAddProduct = (newProd: Product) => {
    // Insert safely in database (digit-normalized comparison prevents
    // Persian/English numeral variants of the same barcode duplicating)
    const cleanNewId = normalizeBarcode(newProd.id).toLowerCase();
    if (products.some((p) => normalizeBarcode(p.id).toLowerCase() === cleanNewId)) return;
    const enriched = {
      ...newProd,
